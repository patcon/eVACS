				     ...
				     struct play_list *head)
{
	...
	fprintf(stderr, "  Got cmd %p: %s\n",
		list,
		cmd == AUDIO_LOOP ? "LOOP" 
		: cmd == AUDIO_PLAY ? "PLAY"
		: cmd == AUDIO_PAUSE ? "PAUSE" 
		: cmd == AUDIO_STOP ? "STOP"
		: cmd == AUDIO_EXIT ? "EXIT"
		: 0);
	...
	if (head) {
		...
		while (end->next) {
			fprintf(stderr, "Iterating through list...\n");
			end = end->next;
		}
		end->next = list;
	} else {
...
static unsigned int get_time_in_buffer(int audio_fd)
{
	...
	if (info.bytes >= bytes_total) {
		char junk[32768] = { 0 };
		fprintf(stderr, "Confused... sweeping...\n");
...
#if 1
			time_in_buffer = get_time_in_buffer(audio_fd);
			fprintf(stderr, "done %u, %u ms in buffer\n", done,
				time_in_buffer);
#else
			/* guesstimate */
