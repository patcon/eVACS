...
#define HOME_CURSOR \
	((struct cursor) { .group_index = 0, .screen_candidate_index = -1 })
			       ...
			       const struct rotation *rot)
{
	...
	dbci = translate_sci_to_dbci(num_candidates(where.group_index),
					    ...
					    struct cursor start)
{
	...
	if (prefs->num_preferences != 0) {
		...
		where.screen_candidate_index
			= translate_dbci_to_sci(num_candidates(where
							       .group_index),
			     ...
			     const struct preference_set *vote)
{
	...
	prefs = recon_keystrokes(rot, keystrokes);
