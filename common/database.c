...
struct polling_place *get_polling_place(PGconn *conn, const char *ppname)
{
	...
	result = SQL_query(conn,
			   "SELECT * from polling_place WHERE name = '%s';",
			   ppname);
...
struct electorate *get_electorates(PGconn *conn)
{
	...
	result = SQL_query(conn,"SELECT * from electorate;");
...
/*
  Function that supports the Election Night system.
*/

void update_vote_summary(PGconn *conn)
/*
  Update the vote_summary and preference_summary tables
  from the confirmed_preference and confirmed_vote tables.

  THIS FUNCTION SHOULD NOT BE CALLED FROM data entry OR load votes!

  This function should only be used to reconstruct the Election Night
  summary tables from the confirmed_vote and confirmed_preference tables.

  This only needs to be done once after Election Night.
*/
{
	/* Ensure a read consistent view of the source tables */
	SQL_command(conn,"SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;");

	/* Create archived copy of votes and preferences */
	SQL_command(conn,"CREATE TABLE archived_vote_summary AS "
		    "SELECT * FROM vote_summary;");
	SQL_command(conn,"CREATE TABLE archived_preference_summary AS"
		    "SELECT * FROM preference_summary;");

	/* Empty the summary tables */
	SQL_command(conn,"TRUNCATE TABLE vote_summary;");
	SQL_command(conn,"TRUNCATE TABLE preference_summary;");

	/* Get first preference counts */
	SQL_command(conn,"INSERT INTO preference_summary "
		    "SELECT cv.electorate_code,cv.polling_place_code,"
		    "cp.group_index,cp.db_candidate_index,count(*) "
		    "FROM confirmed_vote cv,confirmed_preference cp "
		    "WHERE cv.id = cp.vote_id "
		    "AND cp.prefnum = 1 "
		    "GROUP BY cv.electorate_code,cv.polling_place_code,"
		    "cp.group_index,cp.db_candidate_index;");

	/* Get unique electorate/polling place combinations */
	SQL_command(conn,"INSERT INTO vote_summary"
		    "(electorate_code,polling_place_code) "
		    "SELECT DISTINCT electorate_code,polling_place_code "
		    "FROM confirmed_vote;");
	
	/* Count informal votes */
		    ...
		    "(SELECT COUNT(*) "
		     ...
		     "WHERE NOT EXISTS "
		       "(SELECT 1 "
		        ...
		        "WHERE vote_id = cv.id) "
		        "AND cv.electorate_code = electorate_code "
		        "AND cv.polling_place_code = polling_place_code);");

	/* Set timestamp and operator id */
	SQL_command(conn,"UPDATE vote_summary "
		    "SET entered_by = 'EVACS',entered_at = 'NOW';");
