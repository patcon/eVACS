...
static bool vacancy_flag;

bool is_vacancy(void)
{
	return vacancy_flag;
} 

void set_vacancy(bool flag)
{
	vacancy_flag=flag;
}
			       ...
			       struct candidate *vacating)
{
	...
	static bool distribute(struct ballot *ballot, void *vacating)
	{
		unsigned int i,j=0;
	...
	for_each_ballot(ballots, &distribute, (void *) vacating);
}
...
*/
static bool mark_pending_candidates(struct cand_list *candidates,
				    unsigned int quota,
				    struct candidate *vacating)
{
		       ...
		       bool is_last)
{
	...
	/* Calculate majority for this count of the P.Ex */
	if (is_vacancy()) {
		quota = calc_majority(candidates);
		report_majority(get_count_number(), quota);
	}
				  ...
				  struct candidate *vacating)
{
	...
	/* Nested function: gcc extension */
	static bool place_in_pile(struct ballot *ballot, void *unused)
	{
		unsigned int i;
		bool new_pile = true;
