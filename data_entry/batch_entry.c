 /* This file is (C) copyright 2000 Software Improvements, Pty Ltd */
...
static struct predefined_batch *set_batch_number(void)
{
	...
	strcpy(&electorate_name[0],resolve_electorate_name(conn, electorate_code));
...
static bool same_deo(PGconn *conn,unsigned int batch_number)
{
	...
	struct entry *entry;
	char *operator_id = (char*) get_operator_id();
	unsigned int i;
...
/* dont allow a deo to enter a correct batch, or a batch with an error which 
   is not a KEYSTROKE error */
	while (batch_ok == false) {
		...
		batch_with_error = find_errors_in_batch(batch);
		batch_ok = abort_batch_entry(batch_with_error);
		if (!batch_ok) printf ("That batch is either correct or "
...
void batch_entry(void) {
	...
	while (1) {
		...
		if (!conn) bailout("Can't connect to %s at %s\n",
				   DATABASE_NAME,SERVER_ADDRESS);
		entries = (unsigned int)SQL_singleton_int(conn,
					    "SELECT MAX(e.index) "
		...
		if (entries > 1) {
		        print_batch(conn,batch_info->batch_number);
		        print_errors_in_batch(conn,batch_info->batch_number);
		} else {
		        printf("Batch %u entered for the first time.\n"
			       "Nothing sent to printer.\n", batch_info->batch_number);
